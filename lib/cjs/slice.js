    ...
    reducers: {
        ...
        setServiceState: (state, action) => {
            ...
            if (state.serviceState.every((el) => el))
                state.state = constants_1.KiteState.Running;
            else
                state.state = constants_1.KiteState.Paused;
            writeConfigToFile(state);
        },
...
function readConfigFromFile() {
    const defaultState = {
        ...
        kafkaSetup: {},
        dBSetup: {},
        state: constants_1.KiteState.Init,
        serverState: constants_1.KiteServerState.Disconnected,
        configFile: {}, //Promise<KiteConfigFile> | KiteConfigFile;
    };
    try {
        fs_extra_1.default.mkdirSync(path_1.default.resolve(constants_2.configFilePath), { recursive: true });
        const state = fs_extra_1.default.readFileSync(path_1.default.resolve(constants_2.configFilePath, 'cfg.json'), 'utf-8');
...
function writeConfigToFile(state) {
    try {
        ...
        fs_extra_1.default.writeFileSync(path_1.default.resolve(constants_2.configFilePath, 'cfg.json'), JSON.stringify(state));
    }
    catch (err) {
