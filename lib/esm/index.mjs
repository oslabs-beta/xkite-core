...
import { setPackageBuild, setConfig, setServer, setSetup, setState, setServerState, setServiceState, setConfigFile, } from './state/slice';
function KiteCreator() {
    const selectedPorts = new Set();
    async function configServer(server) {
    ...
    async function checkConfigPorts(cfg) {
        try {
            cfg.kafka = {
                ...cfg.kafka,
                brokers: {
                    ...
                    ports: {
                        ...
                        brokers: await checkPorts(cfg.kafka.brokers?.ports?.brokers ??
                            new Array(cfg.kafka.brokers.size).fill(_ports_.kafka.broker.external)),
                        jmx: await checkPorts(cfg.kafka.brokers?.ports?.jmx ??
                ...
                zookeepers: {
                    ...
                    ports: {
                        ...
                        client: await checkPorts(cfg.kafka.zookeepers?.ports?.client ??
                            new Array(cfg.kafka.zookeepers.size).fill(_ports_.zookeeper.client.external)),
                    },
                },
            };
            if (cfg.kafka.jmx !== undefined) {
                cfg.kafka.jmx = {
                    ...cfg.kafka.jmx,
                    ports: await checkPorts(cfg.kafka.jmx.ports ??
            ...
            if (cfg.kafka.spring !== undefined) {
                cfg.kafka.spring = {
                    ...cfg.kafka.spring,
                    port: await checkPort(cfg.kafka.spring.port ?? _ports_.spring.external),
                };
            }
            if (cfg.db !== undefined) {
                if (cfg.db.kafkaconnect !== undefined) {
                    cfg.db.kafkaconnect = {
                        ...cfg.db.kafkaconnect,
                        port: await checkPort(cfg.db.kafkaconnect?.port ?? _ports_.kafkaconnect_src.external),
                    };
                }
                else {
                    cfg.db.kafkaconnect = {
                ...
                cfg.db = {
                    ...cfg.db,
                    port: await checkPort(cfg.db.port ??
                        (cfg.db.name === 'ksql'
                ...
                if (cfg.db.ksql !== undefined) {
                    cfg.db.ksql = {
                        ...cfg.db.ksql,
                        schema_port: await checkPort(cfg.db.ksql.schema_port ?? _ports_.ksql_schema.external),
                    };
                }
            }
            if (cfg.sink !== undefined) {
                if (cfg.sink.kafkaconnect !== undefined) {
                    cfg.sink.kafkaconnect = {
                        ...cfg.sink.kafkaconnect,
                        port: await checkPort(cfg.sink.kafkaconnect.port ?? _ports_.kafkaconnect_sink.external),
                    };
                }
                else {
                    cfg.sink = {
                ...
                if (cfg.sink.name === 'jupyter') {
                    ...
                    cfg.sink = {
                        ...cfg.sink,
                        port: await checkPort(cfg.sink?.port ?? _ports_.spark.webui.external),
                        rpc_port: await checkPort(cfg.sink?.rpc_port ?? _ports_.spark.rpc.external),
                    };
                }
            }
            ...
            if (cfg.prometheus !== undefined) {
                cfg.prometheus = {
                    ...cfg.prometheus,
                    port: await checkPort(cfg.prometheus.port ?? _ports_.prometheus.external),
                };
            }
        }
        catch (error) {
    ...
    async function configLocal(config) {
        await checkConfigPorts(config);
        store.dispatch(setState(KiteState.Init));
        store.dispatch(setServerState(KiteServerState.Disconnected));
        try {
            const generate = ymlGenerator();
    ...
    async function pauseServer(service) {
        try {
            const { server } = store.getState();
    ...
    async function pauseLocal(service) {
    ...
    async function unpauseServer(service) {
        try {
            const { server } = store.getState();
    ...
    async function unpauseLocal(service) {
        if (service === undefined)
            return;
        for (const name of service) {
            try {
    ...
    return {
        ...
        unpause: async function (service) {
            const { serverState, services } = store.getState();
            if (service === undefined)
                service = services;
            if (serverState === KiteServerState.Connected) {
...
export default Kite;
export { defaultCfg, KiteState, KiteServerState, configFilePath, ymlGenerator, downloadDir, _ports_, MAX_NUMBER_OF_BROKERS, MAX_NUMBER_OF_ZOOKEEPERS, };
