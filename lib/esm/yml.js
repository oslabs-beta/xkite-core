...
import os from 'os';
import { YAML, SPRING, SPARK, JUPYTER, KSQL_SCHEMA, KSQL_CLI, KSQL, POSTGRES, GRAFANA, PROMETHEUS, ZOOKEEPER, KAFKA_BROKER, JMX, PROMCONFIG, downloadDir, network, _ports_, KAFKA_CONNECT_SRC, KAFKA_CONNECT_SINK, } from './constants/yml';
const dependencies = [];
const setup = {
    kafkaSetup: {
        clientId: '',
...
const ymlGenerator = () => {
    return (config) => {
        ...
        try {
            ...
            fs.writeFileSync(path.resolve(downloadDir, 'docker-compose.yml'), yaml.dump(YAML, { noRefs: true }));
            if (prometheus !== undefined) {
                ...
                fs.writeFileSync(path.resolve(downloadDir, 'prometheus/prometheus.yml'), yaml.dump(PROMCONFIG, { noRefs: true }));
    ...
    function createDB(db) {
        if (db?.name === 'postgresql') {
            ...
            YAML.services.postgresql = {
                ...POSTGRES,
                ports: [
                    `${db.port ?? _ports_.postgresql.external}:${_ports_.postgresql.internal}`,
                ],
                environment: {
            ...
            YAML.services.ksql = {
                ...
                environment: {
                    ...
                    KSQL_KSQL_SCHEMA_REGISTRY_URL: `http://schema-registry:${db.ksql?.schema_port ?? _ports_.ksql_schema.internal}`,
                },
            };
            YAML.services.ksql_schema = {
                ...KSQL_SCHEMA,
                ports: [
                    `${db.ksql?.schema_port ?? _ports_.ksql_schema.external}:${_ports_.ksql_schema.internal}`,
                ],
            };
        ...
        if (config.sink !== undefined) {
            ...
            if (config.sink?.name === 'jupyter') {
                YAML.services.jupyter = {
                    ...JUPYTER,
                    ports: [
                        `${config.sink?.port ?? _ports_.jupyter.external}:${_ports_.jupyter.internal}`,
                    ],
                };
                setup.jupyter = { port: config.sink?.port ?? _ports_.jupyter.external };
            }
            else if (config.sink?.name === 'spark') {
                YAML.services.spark = {
                    ...SPARK,
                    ports: [
                        `${config.sink?.port ?? _ports_.spark.webui.external}:${_ports_.spark.webui.internal}`,
                        `${config.sink?.rpc_port ?? _ports_.spark.rpc.external}:${_ports_.spark.rpc.internal}`,
                    ],
                };
                setup.spark = {
                    port: [
    ...
    function createZooKeepers(kafka) {
        ...
        const getZKServerPorts = () => {
            ...
            const getPeerPort = (x) => {
                ...
                return `${x + 1}${_ports_.zookeeper.peer.external}:${x + 1}${_ports_.zookeeper.peer.internal};`;
            };
        ...
        for (let i = 0; i < numOfZKs; i++) {
            ...
            if (kafka.zookeepers.ports?.client !== undefined &&
            ...
            if (YAML.services.ksql_schema !== undefined) {
                ...
                YAML.services.ksql_schema.environment.SCHEMA_REGISTRY_KAFKASTORE_CONNECTION_URL += `${name}:${_ports_.zookeeper.client.internal},`;
            }
    ...
    function createBrokers(config, servers) {
        ...
        if (kafka.jmx !== undefined) {
            ...
            jmxExporterConfig = yaml.load(fs.readFileSync(path.resolve(downloadDir, 'jmx/exporter/template.yml'), 'utf8'));
            setup.jmx = { ports: [] };
        }
        ...
        for (let i = 0; i < kafka.brokers.size; i++) {
            ...
            YAML.services[brokerName] = {
                ...
                environment: {
                    ...
                    KAFKA_OFFSETS_TOPIC_REPLICATION_FACTOR: (kafka.brokers.replicas ?? 1) > kafka.brokers.size
                        ? kafka.brokers.size
                        : kafka.brokers.replicas ?? 1,
                    KAFKA_TRANSACTION_STATE_LOG_REPLICATION_FACTOR: (kafka.brokers.replicas ?? 1) > kafka.brokers.size
                        ? kafka.brokers.size
                        : kafka.brokers.replicas ?? 1,
            ...
            if (kafka.jmx !== undefined) {
                ...
                YAML.services[jmxName] = {
                    ...JMX,
                    command: [`${_ports_.jmx.internal}`, '/etc/myconfig.yml'],
                    ...
                    volumes: [
                        `${path.join(downloadDir, `/jmx/jmxConfigKafka${n}.yml`)}:/etc/myconfig.yml`,
                    ],
                    depends_on: [`kafka${n}`],
                };
                setup.jmx?.ports?.push(jmxPort);
                jmxExporterConfig.hostPort = `kafka${n}:${jmxHostPort}`;
                fs.writeFileSync(path.resolve(downloadDir, `jmx/jmxConfigKafka${n}.yml`), yaml.dump(jmxExporterConfig, { noRefs: true }));
                if (config.prometheus !== undefined) {
                    PROMCONFIG.scrape_configs[0].static_configs[0].targets.push(`${jmxName}:${_ports_.jmx.internal}`);
                    PROMCONFIG.scrape_configs[1].static_configs[0].targets.push(`${ipAddress}:${_ports_.docker.internal}`);
                }
            }
            if (YAML.services.ksql !== undefined) {
                YAML.services.ksql = {
                    ...
                    environment: {
                        ...
                        KSQL_BOOTSTRAP_SERVERS: YAML.services.ksql.environment.KSQL_BOOTSTRAP_SERVERS +
            ...
            if (YAML.services.kafka_connect_src !== undefined) {
                ...
                const kcdeps = YAML.services.kafka_connect_src.depends_on === undefined ||
                ...
                YAML.services.kafka_connect_src = {
                    ...
                    environment: {
                        ...kcenv,
                        CONNECT_BOOTSTRAP_SERVERS: kcenv.CONNECT_BOOTSTRAP_SERVERS !== ''
            ...
            if (YAML.services.kafka_connect_sink !== undefined) {
                ...
                const kcdeps = YAML.services.kafka_connect_sink.depends_on === undefined ||
