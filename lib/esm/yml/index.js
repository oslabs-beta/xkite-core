...
import os from 'os';
import { YAML, SPRING, SPARK, JUPYTER, KSQL_SCHEMA, KSQL_CLI, KSQL, POSTGRES, GRAFANA, PROMETHEUS, ZOOKEEPER, KAFKA_BROKER, JMX, PROMCONFIG, downloadDir, network, _ports_, KAFKA_CONNECT_SRC, KAFKA_CONNECT_SINK, } from '../types/yml/constants';
const dependencies = [];
const setup = {
    kafkaSetup: {
        clientId: '',
        ...
        if (db?.name === 'postgresql') {
            ...
            YAML.services.ksql = {
                ...
                environment: {
                    ...
                    KSQL_KSQL_SCHEMA_REGISTRY_URL: `http://schema-registry:${db.ksql?.schema_port ?? _ports_.ksql_schema.internal //TODO revisit/test
                    }`,
                },
            };
        ...
        for (let i = 0; i < kafka.brokers.size; i++) {
            ...
            if (kafka.jmx !== undefined) {
                ...
                if (config.prometheus !== undefined) {
                    ...
                    PROMCONFIG.scrape_configs[1].static_configs[0].targets.push(`${ipAddress}:${_ports_.docker.internal}`); //TO DO: IK: configure the port
                }
            }
